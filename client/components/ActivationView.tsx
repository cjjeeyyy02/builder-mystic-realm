...
const initialEmployees: Employee[] = [
  {
    jobId: "001",
    name: "Jaya Sharma",
    appliedJobRole: "Senior Developer",
    joiningDate: "8/15/2025",
    filesUploaded: "3/5",
    activationProgress: 75,
    finalDecision: "ACTIVATE EMPLOYEE",
  },
  {
    jobId: "002",
    name: "Mark Robinson",
    appliedJobRole: "Graphic Designer",
    joiningDate: "8/15/2025",
    filesUploaded: "4/5",
  ...
  },
  {
    jobId: "003",
    name: "John Carter",
    appliedJobRole: "Content Writer",
    joiningDate: "8/15/2025",
    filesUploaded: "5/5",
    activationProgress: 100,
    finalDecision: "EMPLOYEE ACTIVATED",
  },
  {
    jobId: "004",
    name: "Sara Ahmed",
    appliedJobRole: "Copywriter",
    joiningDate: "8/15/2025",
    filesUploaded: "4/5",
    activationProgress: 90,
    finalDecision: "ACTIVATE EMPLOYEE",
  ...
  },
  {
    jobId: "006",
    name: "Robin Blake",
    appliedJobRole: "AI Engineer",
    joiningDate: "8/15/2025",
    filesUploaded: "1/5",
    activationProgress: 15,
    finalDecision: "ACTIVATE EMPLOYEE",
  },
  {
    jobId: "007",
    name: "Kayle Morgan",
    appliedJobRole: "ML Engineer",
    joiningDate: "8/15/2025",
    filesUploaded: "5/5",
    activationProgress: 100,
    finalDecision: "EMPLOYEE ACTIVATED",
  ...
  },
  {
    jobId: "009",
    name: "Anne Garcia",
    appliedJobRole: "Finance Analyst",
    joiningDate: "8/15/2025",
    filesUploaded: "2/5",
    activationProgress: 40,
    finalDecision: "ACTIVATE EMPLOYEE",
...
export default function ActivationView() {
  ...
  >(() => {
    ...
    initialEmployees.forEach((e) => {
      map[e.jobId] = checklistItems.map((it, idx) => ({
        id: String(idx + 1),
        title: it.title,
  ...
  const [selectedEmployeeForChecklist, setSelectedEmployeeForChecklist] =
  ...
  const handleChecklistFileUpload = (
    jobId: string,
    itemId: string,
    file: File | null,
  ) => {
    ...
    setChecklistMap((prev) => {
      ...
      items[idx] = {
        ...items[idx],
        completed: true,
        files: [...items[idx].files, fileEntry],
  ...
  const handleUpdateItemStatus = (
    ...
    setChecklistMap((prev) => {
      ...
      items[idx] = {
        ...items[idx],
        completed: isPass,
        dateSubmitted: isPass
          ? new Date().toISOString()
          : items[idx].dateSubmitted,
      };
      copy[jobId] = items;
  ...
  const sendGmailTemplate = async (
    action: "approve" | "reject",
    emp: Employee,
  ) => {
    console.log(
      `Would send Gmail template for ${action} to ${emp.name} (job ${emp.jobId})`,
    );
    setEmployees((prev) =>
      prev.map((e) =>
        e.jobId === emp.jobId
          ? {
              ...e,
              activationProgress:
                action === "approve" ? 100 : e.activationProgress,
            }
          : e,
      ),
    );
    ...
    alert(
      `${action === "approve" ? "Approval" : "Rejection"} email triggered for ${emp.name}. (Simulated)`,
    );
  };
  ...
  const renderProgressBar = (progress: number) => {
    ...
    return (
      <div className="flex items-center gap-2">
        <div
          className={`w-24 rounded-full h-2 transition-colors duration-300 ${
    ...
    >
      ...
      {activeTab === "activation-room" ? (
        <div className="space-y-3">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  const rows = filteredEmployees.map((e) => [
                    ...
                    e.name,
                    e.appliedJobRole,
                    formatDateMDY(e.joiningDate),
                    e.filesUploaded,
                    `${e.activationProgress}%`,
                  ]);
                  const csv = [
                    headers.join(","),
                    ...rows.map((r) =>
                  ...
                  a.href = url;
                  a.download = "activation_candidates.csv";
                ...
                <TableBody>
                  {filteredEmployees.slice(0, 5).map((employee) => (
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className="h-6 w-6 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center shadow-sm">
                            <span className="text-xs font-medium text-white">{employee.name.split(" ").map(n=>n[0]).slice(0,2).join("")}</span>
                          </div>
                          <div className="leading-tight min-w-0">
                            <div className={`text-[14px] font-medium ${isDarkMode ? "text-gray-100" : "text-gray-900"}`}>{employee.name}</div>
                      ...
                      <TableCell
                        className={`text-[14px] px-3 py-3 transition-colors duration-300 align-middle ${
                          isDarkMode ? "text-gray-300" : "text-gray-700"
                        }`}
                      >
          ...
          {viewMode === "card" && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {filteredEmployees.slice(0, 5).map((employee) => (
                <div
                  key={employee.jobId}
                  className={`border rounded-md p-3 hover:shadow-lg transition-all duration-200 border-l-4 border-l-transparent hover:border-l-primary/30 ${
                    isDarkMode
                      ? "bg-gray-800 border-gray-700"
                      : "bg-white border-gray-200"
                  }`}
              ...
              {selectedEmployeeForChecklist && (
                <div className="space-y-4">
                  <Accordion type="single" collapsible className="w-full">
                    {(
                      checklistMap[selectedEmployeeForChecklist.jobId] || []
                    ).map((item) => (
                              ...
                              <span
                                ...
                                onClick={(e) => {
                                  ...
                                  setChecklistMap((prev) => {
                                    ...
                                    copy[selectedEmployeeForChecklist!.jobId] =
                                      arr;
                                    return copy;
                                  });
                                }}
                          ...
                          <div className="space-y-3 p-3 rounded-md border bg-background">
                            <div className="flex flex-wrap gap-2">
                              <label className="inline-flex items-center">
                                ...
                                />
                                <span className="inline-flex items-center px-2 py-1 border rounded text-xs cursor-pointer">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  else toast({ title: "No file uploaded yet" });
                                }}
                              >
                              ...
                              <Button
                                ...
                                size="sm"
                                className="text-xs"
                                onClick={() => setStatusModalItemId(item.id)}
                              >
                                Update
                              </Button>
                              <div className="ml-auto text-xs text-muted-foreground">
                                Date Submitted:{" "}
                                {item.dateSubmitted
                                  ? formatDateMDY(item.dateSubmitted)
                                  : "-"}
                              </div>
                            </div>

                            {item.textSubmission ? (
                              <div className="text-sm whitespace-pre-wrap border rounded p-2">
                                {item.textSubmission}
                              </div>
                            ) : (
                    ...
                    return (
                      <div className="flex items-center justify-between pt-2 border-t">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            size="sm"
                            className={`bg-green-600 hover:bg-green-700 h-8 px-3 text-xs ${allAccepted ? "" : "opacity-50 cursor-not-allowed"}`}
                            ...
                            onClick={() => {
                              ...
                              toast({ title: "Activation approved" });
                            }}
                          >
                            Approve Activation
                          </Button>
              ...
              <div className="space-y-3 text-xs">
                ...
                <div>
                  ...
                  <Input
                    type="file"
                    onChange={(e) =>
                      setEmailAttachment(e.target.files?.[0] || null)
                    }
                  />
                </div>
              </div>
          ...
          {/* Update Status Modal */}
          <Dialog
            open={!!statusModalItemId}
            onOpenChange={(open) => !open && setStatusModalItemId(null)}
          >
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle className="text-sm font-semibold">
              ...
              <div className="text-xs text-muted-foreground">
                Mark this checklist item as Reject or Accept.
              </div>
              <DialogFooter className="flex-row justify-end gap-2">
                <Button
                  variant="destructive"
                  ...
                  onClick={() => {
                    if (!selectedEmployeeForChecklist || !statusModalItemId)
                      return;
        ...
        <div className="space-y-3">
          ...
          >
            <Table className="w-full text-sm">
              <TableHeader>
                ...
                >
                  <TableHead
                    className={`text-sm font-semibold px-4 py-3 transition-colors duration-300 ${
                  ...
                  <TableHead
                    className={`text-sm font-semibold px-4 py-3 text-right transition-colors duration-300 ${
                      isDarkMode ? "text-gray-300" : "text-gray-600"
                    }`}
                  >
                    ACTIONS
          ...
          <div
            className={`flex items-center justify-between p-4 border-t  transition-colors duration-300 ${
              isDarkMode
                ? "bg-gray-800 border-gray-700"
